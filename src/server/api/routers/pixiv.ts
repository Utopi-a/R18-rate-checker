...
export const pixivRouter = createTRPCRouter({
  ...
  getIllustCount: publicProcedure.input(z.object({ queries: z.string().array() })).query(async ({ input }) => {
    const { queries } = input;

    const encodedQueries = queries.map((query) => ({
      [query]: { all: encodeURIComponent(query), R18: encodeURIComponent(query + " R-18") },
    }));
    ...
    const totalCounts = await Promise.all(
      encodedQueries.map(async (encodedQuery, index) => {
        ...
        return { [decodeURIComponent(Object.values(encodedQuery)[0]?.all ?? "")]: { all: allCount, R18: r18Count } };
      })
    );
